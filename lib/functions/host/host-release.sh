...
function obtain_and_check_host_release_and_arch() {
	...
	if [[ -z $HOSTRELEASE || "bookworm trixie sid jammy kinetic lunar vanessa vera victoria 36" != *"$HOSTRELEASE"* ]]; then
...
function obtain_hostrelease_only() {
	...
	HOSTRELEASE="$(cat /etc/os-release | grep VERSION_ID | cut -d"=" -f2)"
